 ...
 * callers (`GovernanceService.closeAbstimmung`) write `0` explicitly for every ballot not in
 * [charges] when persisting `settled_ltr`, since the DB column must never stay `null` once an
 * Abstimmung is [network.lapis.cloud.shared.domain.AbstimmungStatus.GESCHLOSSEN].
 */
 ...
 *    flagged for the reviewer); `GovernanceService.closeAbstimmung` maps this to
 *    [network.lapis.cloud.shared.domain.AntragStatus.VERTAGT].
