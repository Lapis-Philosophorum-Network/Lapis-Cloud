...
As of *V0.6.5*, six major waves are complete (see `CHANGELOG.md` for full detail; note the package
version and git tags have not yet been bumped past `v0.5.1` — V0.6's sub-waves have so far shipped
as feature commits, with the version/tag/release choreography apparently deferred until the V0.6
LTR-economy arc is fully done):
...
* **V0.6 — LTR economy (in progress)**: a real, ledger-backed LTR balance (`LtrLedgerEntry`,
  replacing the earlier balance stub), Internes Crowdfunding (member-submitted, board-reviewed
  projects with a silence-is-approval clock, Like/Dislike-driven monthly EUR donation-pool
  distribution), direct member-to-member LTR peer transfers (with a board/admin arbitration-
  correction path), Politiker-Profile und Politiker-Ranking (member-only Like/Dislike trust
  weighting of granted politician status, computed as a single shared LTR pool apportioned
  proportionally across all active politicians), and a Price-Oracle for the anchor-asset peg with a
  load-bearing donation → LTR conversion boundary. V0.6.2 (Auction) remains blocked on legal
  review — the one remaining item of this wave's original scope.
...
rating basket, federation not yet built) are tracked in `CHANGELOG.md`. See
link:docs/architecture/domain-model.adoc[domain model documentation] (covers the V0.1.5
Contributions/Documents/Communication slice, the V0.3 accounting slices, the V0.4.1 mail-merge
slice, and the V0.6.5 Price-Oracle slice — not yet updated for every wave in between, including
V0.6.4) and `CLAUDE.md` for further detail.

CAUTION: The V0.6.4 guest/Gast rating basket cut (`PoliticianProfileDto` exposes
`memberTrustWeight` only, not the concept's three-way Mitglieder-/Gäste-/Gesamtbewertung) is an
*open product decision*, not an accepted final scope narrowing — it still needs an explicit
accept/reject from the product owner. See `CHANGELOG.md` `[0.6.4]` "Known limitations" for the
full reasoning (no operational Gast identity/exit workflow exists anywhere in this codebase to
source a guest basket from). This note is cross-referenced here, not just in the changelog, so it
is not missed before this wave is considered fully closed.
