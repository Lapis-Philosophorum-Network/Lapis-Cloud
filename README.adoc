...
As of *V0.7.3* (tag `v0.7.1`), seven major waves are complete (see `CHANGELOG.md` for full
detail):
...
* **Two known, disclosed, not-yet-fixed gaps found during review** (tracked as follow-up waves):
  `PeerTransferService.transferLtr`/`GovernanceService.castVoteBallot` don't gate on active
  membership (an `ANTRAG` applicant could in principle stake/transfer LTR before board approval);
  and RPC error responses for `@RpcServiceException` subclasses (defined in the JVM-only
  `lapis-server` module) fail to deserialize cleanly on the Kotlin/JS client — the authorization
  boundary itself is unaffected, only the error's wire shape is wrong.
